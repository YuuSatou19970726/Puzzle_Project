    ...
    public class GridSetComparer : IEqualityComparer<GridList>
    {
        ...
        public bool Equals(GridList x, GridList y)
        {
            ...
            for (int i = 0; i < posKeys.Length; i++)
            {
                ...
                if ((firstGrid[pos] == -1 && secondGrid[pos] != -1) || (firstGrid[pos] != -1 && secondGrid[pos] == -1))
                    return false;
            ...
            for (int i = 0; i < GridData.LevelSize; i++)
            {
                for (int j = 0; j < GridData.LevelSize; j++)
                {
                    ...
                    if (obj.Data._grid[startPos] != -1)
                    {
                        ...
                        {
                            checkPos = startPos + d;
                            if (obj.Data.IsInsideGrid(checkPos) && obj.Data._grid[checkPos] == obj.Data._grid[startPos])
                            {
                                resultString += "1";
                            }
                            else
    ...
    public class GridData
    {
        ...
        private void Rotate()
        {
            ...
            for (int i = 0; i < LevelSize; i++)
            {
                ...
                {
                    result[Rotate(new Point(i, j))] = _grid[new Point(j, j)];
                }
            }
